...
```bash
npx --no-audit -y pdcaw@1 --help
```

**`--no-audit -y`는 장식이 아니다.** `pdcaw@1`처럼 범위로 부르면 npm이 실행마다 레지스트리를 재확인하고 트리를 다시 짜면서(로그의 `REPLACE`) 감사를 또 돈다. 감사가 막히면 npm 기본값인 `fetch-timeout` 300초 × 재시도 2회를 다 쓰고서야 경고를 찍고 넘어간다 — 스킬 세션이 몇 분씩 멎는 것처럼 보이는 원인이 이것이다. `--no-audit`이 그 왕복을 없애고, `-y`는 첫 설치 때 확인 프롬프트에서 멎는 걸 막는다. 이 레포의 스킬 문서가 CLI를 부르는 모든 줄에 이미 박아뒀다.

머신 전역으로 끄고 싶으면 `~/.npmrc`에 한 줄이면 된다. 스킬 문서의 플래그와 겹쳐도 무해하다.

```
audit=false
```

`npx --no-audit -y pdcaw@1.0.0`처럼 버전을 **정확히** 고정하면 두 번째 실행부터는 트리 재구성 자체를 건너뛴다. 다만 첫 설치 한 번은 여전히 감사를 타므로 `--no-audit`이나 `audit=false`와 같이 써야 한다.
...
아래는 `enhance-lyric-sync`라는 사이클을 `v1.2.0`으로 돌리는 예. 각 단계에서 **사용자가 치는 것**과 **스킬이 하는 것**, **사용자가 기다리는 것**을 갈랐다.
...
사용자는 안 하나를 고른다. 이 시점에 버전과 사이클명이 확정된다. 백로그를 바꾸거나 문서를 쓰지 않는다.
...
사용자가 하는 것: plan과 같은 웹 검수 루프. "확정"으로 끝.
...
10. `backlog-sync`: report §3을 읽어 백로그를 done / resolved / 신규 / 갱신. 폐기와 todo 복원은 사용자만 한다. 판단이 갈리는 건 질문으로 남긴다.

사용자가 하는 것: 7과 9에서 확인. 10의 질문에 답.
...
### 4.1 테스트는 언제 만들고 고치고 없애나

이유 없는 테스트는 없다. 테스트 하나는 SC 하나, 배치 검증 방법 하나, 또는 결함 하나를 근거로 산다. 자동 테스트 수단이 RULE.md에 있는 프로젝트에 적용된다.

| 언제 | 누가 | 하는 일 |
|---|---|---|
| plan | 스킬 | 이 영역을 덮는 기존 테스트를 실측(F-n)에 적는다. SC에 "자동 테스트"를 붙일 때 기존이 덮는지 새로 쓸지가 여기서 갈린다 |
| design | 스킬 | 테스트 파일도 변경 지점(신설 · 수정 · 삭제)에 올린다. 배치마다 "어느 테스트가 무엇을 단언하는가". 동작이 바뀌어 깨질 기존 테스트는 미리 "수정"으로 지목 |
| do, 만들기 | 스킬 | 배치 안에서 코드와 함께. 테스트 통과가 배치 완료 조건. design에 없는 테스트를 추가하면 do §2에 이유를 남긴다 |
| do, 고치기 | 스킬 | design이 "수정"으로 지목한 것만. **그 밖의 실패 테스트를 통과시키려는 수정은 무조건 질문 파일.** 테스트가 틀린 게 뻔해 보여도. 그게 기준을 고치는 순간이라 사람이 본다 |
| do, 없애기 | 스킬 | design이 "삭제"로 지목한 것만 |
| do, 금지 | | skip · quarantine · 조건부 건너뛰기로 초록 만들기. 불안정 테스트는 기록만 하고 close가 백로그로 보낸다 |
| close | 스킬 | SC에 붙은 테스트가 존재하는지가 아니라 **그 SC를 실제로 단언하는지** 읽고 재실행. 결함을 고칠 땐 그걸 잡았을 회귀 테스트를 함께 |
| 패치 | 사용자 · 스킬 | 재현 테스트를 먼저 빨갛게 만들고 수정으로 초록을 만든다 |
| CI 설정 | | 코드다. design 변경 지점이거나 패치. 사이클 중 몰래 바꾸지 않는다 |

CI는 사이클 브랜치 푸시마다 전체 테스트를 돈다. 느리거나 외부 자원이 필요한 테스트는 RULE.md에 별도 명령으로 분리한다. 테스트 위치와 명령은 RULE.md 검증 수단 표가 안다.
...
**plan과 design이 뭐가 다른가.** 독자가 다르다. plan은 사용자가 읽고 "이 범위에 시간을 쓴다"를 정한다. design은 맥락 없는 Sonnet이 읽고 질문 없이 구현한다. design이 plan을 다시 쓰고 있으면 잘못 쓴 것이다.
