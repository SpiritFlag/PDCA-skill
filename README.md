...
```
새 세션 (Opus. adopt · explore 사이클이면 Fable)
> /pdca-design v1.2.0
```
...
- 구현 배치(B-n). 배치마다 "무엇을 확인하면 끝인가"가 붙는다.
- 경로만 보고하고 멈춘다.
...
```
새 세션 (Sonnet)
> /pdca-do v1.2.0          다음 미착수 배치부터
> /pdca-do v1.2.0 B3       특정 배치
```

스킬이 하는 것:
- design과 do.md(있으면)를 읽고 맡을 배치를 정한다.
